...
import { Check, Copy, Eye, EyeOff, GripVertical, Pencil, Plus, Trash2, X } from "lucide-react";
...
export function TestEnvironmentManager({ adminMode = false }: { adminMode?: boolean }) {
  ...
  const canReorder = adminMode && !keyword.trim() && editing.size === 0 && !ordering;
  ...
  async function reorder(targetId: string) {
    if (!draggedId || draggedId === targetId || ordering) return;
    const original = environments;
    const fromIndex = original.findIndex((item) => item.id === draggedId);
    const toIndex = original.findIndex((item) => item.id === targetId);
    ...
    const response = await fetch("/api/test-environments/order", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ids: next.map((item) => item.id) })
    });
    const result = await response.json().catch(() => ({})) as { warning?: string; message?: string };
    setOrdering(false);
    if (!response.ok) {
      setEnvironments(original);
      showToast(result.message ?? "排序保存失败，请重试。", "error");
      return;
    }
    showToast(result.warning ?? "排序已保存。", result.warning ? "warning" : "success");
  }
    ...
    <section>
      ...
      {!loading ? <div className="overflow-x-auto"><Table className="mt-6 min-w-[1460px] table-fixed"><colgroup><col className="w-[180px]" /><col className="w-[160px]" /><col className="w-[120px]" /><col className="w-[300px]" /><col className="w-[160px]" /><col className="w-[180px]" /><col className="w-[240px]" /><col className="w-[180px]" /></colgroup><TableHeader><TableRow className="border-foreground/[0.14]"><TableHead>产品名称</TableHead><TableHead>客户版本</TableHead><TableHead>环境类型</TableHead><TableHead>环境地址</TableHead><TableHead>账号</TableHead><TableHead>密码</TableHead><TableHead>说明</TableHead><TableHead className="text-right">操作</TableHead></TableRow></TableHeader><TableBody>
        {filtered.map((item) => {
          ...
          return <Fragment key={item.id}><TableRow draggable={canReorder && !isEditing} onDragStart={() => setDraggedId(item.id)} onDragEnd={() => { setDraggedId(null); setDragOverId(null); }} onDragEnter={() => { if (canReorder && draggedId !== item.id) setDragOverId(item.id); }} onDragOver={(event) => { if (canReorder) event.preventDefault(); }} onDrop={() => void reorder(item.id)} className={`${draggedId === item.id ? "opacity-40" : canReorder && !isEditing ? "cursor-grab active:cursor-grabbing" : ""} ${dragOverId === item.id ? "border-t-2 border-t-foreground" : ""}`}>
            <TableCell>{isEditing ? <Input controlSize="sm" value={draft.productName} onChange={(event) => update(item.id, "productName", event.target.value)} /> : <div className="flex items-center gap-1"><GripVertical size={15} className={canReorder ? "shrink-0 text-muted-foreground" : "hidden"} /><span className="block truncate" title={item.productName}>{item.productName}</span></div>}</TableCell>
