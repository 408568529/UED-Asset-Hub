...
export function TestEnvironmentForm({ environmentId }: { environmentId?: string }) {
  ...
  return (
    <form onSubmit={submit} className="mt-10 max-w-2xl space-y-5 border-t border-foreground/10 pt-8">
      ...
      <LabeledField label={environmentId ? "测试密码（留空则不修改）" : "测试密码"} required={!environmentId}><Input value={value.password ?? ""} onChange={(event) => update("password", event.target.value)} required={!environmentId} /></LabeledField>
