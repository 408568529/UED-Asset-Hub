...
async function getErrorMessage(response: Response, fallback: string) {
  ...
  if (response.status === 413) return "文件过大，主机或代理拒绝了本次上传。";
  ...
  async function submit(formData: FormData) {
    ...
    try {
      if (!isEdit) {
        ...
        if (!response.ok) {
          const errorMessage = await getErrorMessage(response, "字体上传失败");
      ...
      if (!metaResponse.ok) {
        const errorMessage = await getErrorMessage(metaResponse, "字体信息保存失败");
        setMessage(errorMessage);
        showToast({ message: errorMessage, tone: "error" });
        return;
      }
      ...
      if (file instanceof File && file.size > 0) {
        ...
        if (!versionResponse.ok) {
          const errorMessage = await getErrorMessage(versionResponse, "元数据已保存，但字体新版本上传失败");
          setMessage(errorMessage);
          showToast({ message: errorMessage, tone: "error" });
          return;
        }
      ...
      const errorMessage = "上传请求中断，请检查网络、主机上传大小限制或稍后重试。";
      setMessage(errorMessage);
      showToast({ message: errorMessage, tone: "error" });
    } finally {
  ...
  return (
    <form action={submit} className="mt-10 max-w-2xl space-y-5 border-t border-foreground/10 pt-8">
      ...
      <LabeledField label="字体分类">
        <Select name="category" defaultValue={font?.category ?? "中文字体"}>
      ...
      <LabeledField label={isEdit ? "上传新版本文件（可选）" : "字体文件"}>
        <Input name="file" type="file" required={!isEdit} accept=".otf,.ttf,.woff,.woff2,.zip" />
      </LabeledField>
      ...
      <LabeledField label="标签（可选）">
        <TagMultiSelectField type="font-tag" name="tags" defaultValue={font?.tags} />
      </LabeledField>
      ...
      <div className="flex gap-3">
        <Button type="submit" disabled={isSubmitting}>{isSubmitting ? "上传中..." : isEdit ? "保存字体" : "发布字体"}</Button>
