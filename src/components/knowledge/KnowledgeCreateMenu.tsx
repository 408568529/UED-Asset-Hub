    ...
    <div ref={rootRef} className="relative">
      ...
      {open ? (
        <div role="menu" className="absolute right-0 top-12 z-30 w-72 border border-border bg-[hsl(var(--surface-raised))] p-2 shadow-[var(--shadow-raised)]">
          {actions.map((action) => (
            <Link key={action.href} href={action.href} role="menuitem" onClick={() => setOpen(false)} className="block border-b border-border px-4 py-3 last:border-b-0 transition-colors hover:bg-[hsl(var(--surface-subtle))] focus-visible:bg-[hsl(var(--surface-subtle))]">
