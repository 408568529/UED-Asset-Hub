...
export function TrainingForm({ video, initialFolderName = "" }: { video?: TrainingVideo; initialFolderName?: string }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!video && sourceMode === "upload" && !videoFile) { setToast({ message: "请选择视频文件。", tone: "error" }); return; }
    if (!video && sourceMode === "server-local" && !serverFile) { setToast({ message: "请选择服务器媒体目录中的视频。", tone: "error" }); return; }
    setSubmitting(true);
