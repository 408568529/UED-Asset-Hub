...
export default async function AdminPage() {
  const [products, components, sops, skills, logs, uploads, versions] = await Promise.all([
        ...
        <div className="grid gap-8 md:grid-cols-[1fr_auto] md:items-end">
          <div>
            ...
            <p className="mt-6 max-w-2xl text-lg leading-8 text-muted-foreground">
              管理 Vibe Product、组件规范和标准 SOP。普通列表页只负责浏览，编辑和删除集中在这里。
            </p>
          </div>
          <AdminToolbar />
        </div>
        <AdminOverview productCount={products.length} componentCount={components.length} sopCount={sops.length} skillCount={skills.length} />
        <AdminTabs />
        <div className="mt-12">
          <h2 className="mb-5 text-3xl font-black">资产列表</h2>
          <AdminList products={products} components={components} sops={sops} skills={skills}>
